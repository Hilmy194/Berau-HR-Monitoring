...
import { getOrganizationStructureBreadcrumb, type OrganizationStructureSource } from "@/lib/services/organization-structure.service";
...
export async function GET(request: Request, { params }: { params: Promise<{ code: string }> }) {
  ...
  try {
    return cachedHrCoreJson(await getOrganizationStructureBreadcrumb(parsed.data, source));
  } catch (error) {
